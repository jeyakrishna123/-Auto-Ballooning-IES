...
namespace BubbleDrawing
{
    ...
    public class CommonMethods
    {
        ...
        string AssignMinMaxValue_Degree(string ocrtext)
        {
            ...
            var MinMaxAngles = config.GetSection("AppSettings").GetValue<string>("MinMaxAngles");
            //string MinMaxAngles = appset("").GetValue;// Convert.ToString(_iconfig.GetSection("MinMaxAngles"));
            ...
            MatchCollection match = Regex.Matches(ocrtext, @"\bX?\b");
            if (match != null)
            {
                if(match.Count() > 1)
                {
        ...
        public string AssignMinMaxValue(string ocrtext)
        {
            ...
            var config = new ConfigurationBuilder()
            .AddJsonFile("appsettings.json").Build();
            decimal MinMaxOneDigit = Convert.ToDecimal(config.GetSection("AppSettings").GetValue<string>("MinMaxOneDigit"));
            decimal MinMaxTwoDigit = Convert.ToDecimal(config.GetSection("AppSettings").GetValue<string>("MinMaxTwoDigit"));
            decimal MinMaxThreeDigit = Convert.ToDecimal(config.GetSection("AppSettings").GetValue<string>("MinMaxThreeDigit"));
            decimal MinMaxFourDigit = Convert.ToDecimal(config.GetSection("AppSettings").GetValue<string>("MinMaxFourDigit"));
            ...
            if (xmlLines.Length == 1 && (inputStr.Contains("R.") || inputStr.Contains("R") || inputStr.Contains("Ø") || inputStr.Contains("(") || inputStr.Contains(")") || inputStr.Contains("¡")))
            {
                ...
                int i = 0;
                //while (Math.Round(Convert.ToDecimal(xmlLines[0]), i) != Convert.ToDecimal(xmlLines[0]))
                //    i++;
            ...
            else if (xmlLines.Length == 2 && inputStr.Contains("X"))
            {
                ...
                //output = Convert.ToDecimal(nominal);
                int i = 0;
                //while (Math.Round(Convert.ToDecimal(xmlLines[1]), i) != Convert.ToDecimal(xmlLines[1]))
                //    i++;
            ...
            else if (inputStr.Contains("+") || inputStr.Contains("-"))
            {
                if (xmlLines.Length == 3)
                {
                    //double nominal = Convert.ToDouble(xmlLines[1]);
                    //double mini = Convert.ToDouble(xmlLines[1]) - Convert.ToDouble(xmlLines[2]);
                    //double maxi = Convert.ToDouble(xmlLines[1]) + Convert.ToDouble(xmlLines[0]);
            ...
            //{
            //    cmbType.Text = "Dimension";
            //    cmbSubType.Text = "Linear Dimension";
            //    cmbUnits.Text = "Inches";
            //    cmbTolerance.Text = "Linear";
            //    txtTolearance_Plus.Text = linear_max;
            //    txtTolerance_Minus.Text = linear_min;
            //}
        ...
        Common.ErrorLog objErrorLog = new Common.ErrorLog();
        ...
        public DataTable InitializeBalloonInline()
        {
            ...
            try
            {
                ...
                dtBalloonInline.Columns.Add("BaloonDrwID", typeof(Int64));
